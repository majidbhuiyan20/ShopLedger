...
class ResendOtpViewModel extends StateNotifier<ResendOtpState>{
  final AuthRepository repository;
  ResendOtpViewModel(this.repository) : super(ResendOtpState());

  Future<void> verifyOtp(String email)async{
    state = state.copyWith(isLoading: true, error: null);
    try {
      final request = ResendOtpRequest(email: email.trim());
      final response = await repository.resendOtp(request);
      if (response.isSuccess) {
        state = state.copyWith(isLoading: false, isSuccess: true);
      }
      else{
...
final resendOtpViewModelProvider = StateNotifierProvider<ResendOtpViewModel, ResendOtpState>((ref){
  final repository = ref.read(authRepositoryProvider);
  return ResendOtpViewModel(repository);
});
